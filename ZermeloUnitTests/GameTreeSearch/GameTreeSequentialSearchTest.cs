...
namespace ZermeloUnitTests.GameTreeSearch
{
    public class GameTreeSequentialSearchTest
    {
        ...
        public void GameTreeSequentialSearch_3()
        {
            ...
            for (var i = 0; i < practiceBoard.Player1Pieces.Length; i++)
            {
                var piece = practiceBoard.Player1Pieces[i];
                if (piece.X == 1 && piece.Y == 3)
                {
                    piece.AvailableMoves = new Cell[] { new Cell(0, 2), new Cell(2, 2) };
                }
                if (piece.X == 3 && piece.Y == 3)
                {
                    piece.AvailableMoves = new Cell[] { new Cell(2, 2) };
                }
            }
